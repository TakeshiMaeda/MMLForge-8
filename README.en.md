...
- **Workspace** — The screen is laid out like a DAW: a transport bar with play, stop, loop and volume at the top, a hint bar
...
- **Player** — A line that starts at the left edge is one channel (channels play together for chords and accompaniment).
  A line that starts with a space continues the channel above, so a channel can span as many lines as you like. Loop playback. Your text is saved automatically.
  Comments are `;` (to the end of the line, also mid-line) and `/* ... */` (can span lines).
  Syntax errors show the line and column in the text area, like `Channel 2, line 3, col 5: Unexpected character: "%"`
...
- **Channel on/off** — Checkboxes choose which channels play.
...
- **Optimize** — Tidies the selection (or the whole text) into a more readable form: `c8` → `c` where `l8` is in effect (dots stay),
  ...
  If a part with no `l` has quarter notes, it puts `l4` at the head of the channel and drops the numbers (`c4 d4 e4 f4` → `l4 c d e f`).
  **The first `o` in a channel is kept** (so the channel never leans on the implicit `o4`), and octaves before it are left alone.
...
- **Copy for mml.js** — Removes comments, joins continuation lines and copies one line per channel.
...
| `t140` | Tempo in BPM (default 120; write it in every channel) |
...
| `[ ... ]3` | Repeat 3 times (can nest). An error if one channel expands to more than 1,000,000 notes and rests |
| `[ ... ]` / `[ ... ]0` | No number or 0 = infinite loop: from the second time on, playback repeats from here (for loops with an intro; only at the end of a channel) |
...
- An infinite loop example: `t120 intro [body]0` — the intro plays once, then the body repeats
  (with loop playback off, it stops after one pass of the body). If several channels have one, the latest start point becomes the
...
Just copy the single file `mml.js` and load it (no dependencies, about 560 lines). The global `MMLPlayer` has these 9 members (for the algorithm's details, see the comments in `mml.js` itself — the source is public).

| Member | Type | Description |
|---|---|---|
| `play(channels, opts?)` | `(string \| string[], { loop?: boolean }) => { duration, loopStart }` | Starts playback. `channels` is one channel's MML string, or an array of them (each item is one channel, sounding together). `opts.loop` (default `true`) turns loop playback on or off. In the return value, `duration` is the length of the first pass in seconds, and `loopStart` is where the infinite loop `[ ]0` begins, in seconds (`null` if unused). On a parse failure it throws an `MMLError` and leaves any playing song alone |
| `stop()` | `() => void` | Stops playback, fading out briefly first to avoid clicks |
| `setVolume(v)` | `(number) => void` | Sets the master volume (clamped to 0-1) |
| `setTrackMute(i, muted)` | `(number, boolean) => void` | Mutes or unmutes, at once, the `i`-th item (0-based) of the array passed to `play()`, while it is playing |
| `parse(src, track?)` | `(string, number?) => { notes, duration, tempo, loopStart }` | Parses one channel's MML without playing it (for composing tools). `track` is the number attached to any error (default 0). `notes` is an array of `{time,dur,midi}` |
| `playing` | `boolean` (read-only) | Whether it is currently playing |
| `volume` | `number` (read-only) | The current master volume (0-1) |
| `MMLError` | class | The error type for notation errors, checkable with `instanceof`. Has `code` / `params` / `track` / `pos` / `message` (see "Handling syntax errors" below) |
| `version` | `string` | The engine's version (e.g. `'1.0.0'`, no leading `v`) |
...
<script>
  // Play (each array item is one channel; they sound together)
  MMLPlayer.play([
  ...
  MMLPlayer.setTrackMute(0, true);  // mute channel 0 at once (false to unmute; works while playing)
  MMLPlayer.playing;          // whether it is playing
  MMLPlayer.parse('t90 cde'); // parse only → { notes:[{time,dur,midi}], duration, tempo, loopStart }
                              // a channel number (0-based) as the 2nd argument goes into the error's track
...
try {
  ...
  if (e instanceof MMLPlayer.MMLError) {
    ...
    console.log(e.track);   // 0 … which channel (null for errors about the whole song)
...
- Your app builds the message from `code`. In this repository, `js/mml-messages.js` holds the Japanese and English tables,
  and `locateError` in `js/core.js` adds the line and column to make `Channel 2, line 3, col 5: Unexpected character: "%"`
...
**Comments (`;` `/* */`) and channel splitting (left edge / continuation lines) are features of `index.html`, not of `mml.js`.**
Before passing text to MMLPlayer, the editor blanks out the comments and splits the one text area into an array of channel strings.
...
| Comments `;` `/* ... */` | **×** | ○ |
| Left edge = new channel / leading space = continuation | **×** (pass an array) | ○ |

To take a song written in the editor elsewhere, use **"Copy for mml.js"** in the Editor window.
It copies the song with comments removed and continuation lines joined, one line per channel, so each line can go straight into
the array for `MMLPlayer.play()`. If you also want the editor's notation, copy `stripComments()` / `parseChannelBlocks()` from
...
| File | Contents |
|---|---|
| `elven-morning.mml` | Morning in an elven town in another world. A Celtic-style jig in 6/8, 6 channels, 2:36 (the body loops forever) |
| `slow-blues-in-a.mml` | A 12-bar blues in A, a slow 12/8 shuffle backing. 6 channels, 48 s |
...
- The editor's text handling (comment removal, channel splitting, the bar check and its report)
...
- Lengths, bar counts and equal channel lengths of `songs/*.mml`
...
parser lets it through, and you notice only when the channels drift apart during playback.

"Check bars" on the player row checks whether each channel is a whole number of bars and whether all channels are the same length,
and if not, shows **the line and bar position where a channel first drifts off the bar lines**. Set how many quarter notes make
one bar yourself (4 for 4/4, and 3 for 6/8, since six eighths = three quarters).

In a channel with an infinite loop `[ ]0`, the body is tiled to the end of the song during playback. The bar check also counts
...
A channel whose tempo changes partway cannot have a single bar length, so it is **reported as not checkable** (rather than
