...
function loadCore(MMLPlayer, opts = {}) {
  ...
  const api = eval(src   // eslint-disable-line no-eval
    + ';({ LANG, T, TEXT, APP_VERSION, versionText, stripComments, parseChannelBlocks, channelPos, locateError, mmlMessage, MML_MSG, barCheck, barReport, playStatusText, optimizeMML, ta })');
...
function loadDesk() {
  ...
  return eval([read('js/i18n.js'), read('js/desk.js')].join(';' + String.fromCharCode(10))   // eslint-disable-line no-eval
    + ';({ DESK_WINS, DESK_GAP, DESK_BAR, DESK_KEEP, DESK_NARROW, deskDefaultLayout, deskClamp, deskSnap, deskArrange, deskParse, deskIsOpen, deskIsFront })');
}
